...
#version 4.4 (20230215): added the 2d spectra extraction stand alone panel, to correct and extract long-slit 1d long-slit spectra from a 2d fits wavelength calibrated image.

#version 4.3 (20230207): added the plot window to plot the results generated by SPAN and the match rows in the text editor to match and merge the rows of two ASCII files space separated with one column in common.

#version 4.2 (20231203): added the fits header editor button, with three possible operations on the fits files: modify the keyword of a single file, of a list of files with a list of keywords and extract a keyword from a list of fits.

#version 4.1 (20231202): added the blackbody fitting task

#version 4.0 (20231123): changed the layout of the spectral analysis, added the terminal, fixed some bugs, fixed the progress bar, added the text editor and the menù bar.

#version 3.5 (20221120): added the ppxf algorith to kinematics and stellar populations

#version 3.4 (20221114): added the Line(s) fitting task and corrected minor bugs

#version 3.3 (20221109): modified the Cross-correlation calculation and corrected a bug that crashed the program if two spectral analysis taks were selected. Inserted check and existing conditions on the spectral analysis files and values.

# version 3.2 (20221106): added the tooltip to the tasks/buttons to better clarify that they are doing

# version 3.1 (20221105): rearranged the GUI, fixed a bug in the sigma coeff calculator.

# version 3.0 (20221104): inserted the key values instead of increasing numbers in the GUI. So now you can just add and modify entries without changing ALL the value numbers!

# version 2.95 (20220128): added the calculation of the EW in magnitude for the absurve way to measure some indices in magnitude for no sense at all!

#version 2.9 (20220119): added the SNR determination in the EW task. Now when you measure the EW the program gives you also the SNR calculated in the pseudo-continuum bands.

#version 2.7 (20211124): Adjusted the Sigma coeff and EW correct tasks to be really standalone, with proper cycle and progress bar.

#version 2.6 (20211123): Adjusted the position of the preview result button to fit the shitty resolution of my screen after upgrade of pysimplegui that enlarged automatically (and without a reason) my window.

#version 2.5 (20211123): Added a progress meter for the Process All event

#version 2.4 (20211122): You can now finally process just one spectrum!

#version 2.3 (20211122): Added the degrade function in lambda (FWHM) to degrade the spectrum to a certain FWHM in Angstrom

#version 2.2 (20210927): Adjusted the utility menu and added the possibility to save to a file the SNR of the spectra.

#version 2.1 (20210916): Arranged the utility menu and introduced the conversion flux from Jansky to MKS and MKS f_lambda

#version 2.0 (20210630): Added 'compare spectra' utility, improved the sigma clipping, now called clean task, rearranged the tasks in the pre-processing and processing frames, iproved the plot for the velocity dispersion measure and put a check on the existence of the spectra loaded.

#Version 1.9 (20210626): some make-up to the graphical layout

#Version 1.8 (20210626): add save plot option spectral analysis frame and improved plots of the sigma coeff determination task

#Version 1.7: changed the file names of the result files: now they have a timestamp in order to not overwrite the older files.

#Version 1.6: added the task to measure the resolution of the selected spectra in the Utility tab.
####################################################################################
...
def save_settings(filename, keys, events, values):
    data = {
        ...
        'values': {
            ...
            #'one_spec': values.get('one_spec', False),
            ...
            'clip_factor': values.get('clip_factor', ''),
            'res_spec_for_sigma_clip': values.get('res_spec_for_sigma_clip', ''),
            'single_vel_clip': values.get('single_vel_clip', False),
            'clip_to_vel': values.get('clip_to_vel', ''),
            'file_for_clip': values.get('file_for_clip', False),
            'sigma_clip_file': values.get('sigma_clip_file', ''),
            'dopcor': values.get('dopcor', False),
            'file_for_dopcor': values.get('file_for_dopcor', False),
            'dopcor_file': values.get('dopcor_file', ''),
            'dopcor_single_value': values.get('dopcor_single_value', False),
            'dopcor_value': values.get('dopcor_value', ''),
            'helio_corr': values.get('helio_corr', False),
            'file_for_helio': values.get('file_for_helio', False),
            'helio_file': values.get('helio_file', ''),
            'helio_single_value': values.get('helio_single_value', False),
            'helio_location': values.get('helio_location', ''),
            'helio_date': values.get('helio_date', ''),
            'helio_ra': values.get('helio_ra', ''),
            'helio_dec': values.get('helio_dec', ''),
            'rebin': values['rebin'],
            ...
            'degrade_from_r': values['degrade_from_r'],
            'degrade_to_r': values['degrade_to_r'],
            'degrade_resolution_lambda': values['degrade_resolution_lambda'],
            'degrade_from_l': values['degrade_from_l'],
            'degrade_to_l': values['degrade_to_l'],
...
listbox1 = ['Load a spectra file list and click Load spectra ']

#In you want to change the colors of the panel
#sg.ChangeLookAndFeel('GreenTan')   
sg.SetOptions(tooltip_time=1000) #tooltip time after mouse over
#scaling_factor = 1.25  # Modifica il valore in base alle tue esigenze
#sg.change_look_and_feel(scale=scaling_factor)
font = ("helvetica", 14)
#Builging the main panel#
sg.set_options(auto_size_buttons=True, font=('Helvetica', 14))
...
layout = [
            ...
            #Utility frame
            sg.Frame('Utilities', [
            [sg.Checkbox('HDR', font = ('Helvetica', 14, 'bold'), key = 'show_hdr',tooltip='Show fits header'), sg.Checkbox('Step', font = ('Helvetica', 14, 'bold'), key = 'show_step',tooltip='Show spectrum wavelength step'), sg.Checkbox('Res.:', font = ('Helvetica', 14, 'bold'), key = 'show_res',tooltip='Show resolution, by fitting a sky emission line within the wavelength 1(W1) and wavelength 2(W2) values'),sg.Text('W1'), sg.InputText('670', size = (4,1), key = 'lambda_res_left'), sg.Text('W2'), sg.InputText('673', size = (4,1), key = 'lambda_res_right')],
            [ sg.Checkbox('Convert spectrum to:', font = ('Helvetica', 14, 'bold'), key = 'convert_spec',tooltip='Convert one or all the spectra from fits to ASCII and viceversa'), sg.Radio('Text', "RADIOCONV", default = True, key = 'convert_to_txt'), sg.Radio('FITS', "RADIOCONV", key = 'convert_to_fits')],
            [sg.Checkbox('Compare with: ', font = ('Helvetica', 14, 'bold'), key = 'compare_spec',tooltip='Compare the selected spectrum with any other loaded spectrum'), sg.InputText('Spec.', size = (7,1), key = 'spec_to_compare'), sg.FileBrowse(tooltip='Load the 1D spectrum (ASCII or fits)to use as comparison')],
            [sg.Checkbox('Convert Flux', font = ('Helvetica', 14, 'bold'), key = 'convert_flux',tooltip='Convert the flux from Jansky to F_lambda and viceversa'), sg.Radio('Jy-->F_nu', "FLUX", default = True, key = 'convert_to_fnu'), sg.Radio('Jy-->F_l', "FLUX", key = 'convert_to_fl', font = ('Helvetica', 14,)),sg.Button('See plot',button_color=('black','light gray')), sg.Text(' ', font = ('Helvetica', 1)) ],
            [sg.Checkbox('SNR:', font = ('Helvetica', 14, 'bold'), key = 'show_snr',tooltip='Show the S/N of the spectrum/spectra centered on an user defined wavelength(W)'), sg.Text(' W.'), sg.InputText('852', size = (4,1), key = 'wave_snr'), sg.Text('+/-'), sg.InputText(0.5, size = (3,1), key = 'delta_wave_snr'), sg.Button('Save one',button_color=('black','light gray')), sg.Button('Save all',button_color=('black','light gray'))]  ], font=('Helvetica', 16, 'bold'),tooltip='This is a standalone frame. Use the buttons on the right to perform the tasks'),
            ...
            [sg.Checkbox('Clean', font = ('Helvetica', 14, 'bold'), key = 'sigma_clip',tooltip='Perform sigma clipping to erase spikes.'), sg.Text('Clip ',tooltip='Clipping factor'), sg.InputText(2.5, size = (3,1), key = 'clip_factor'), sg.Text('Res. (R)',tooltip='Spectrum resolution'), sg.InputText('5000', size = (5,1), key = 'res_spec_for_sigma_clip'), sg.Radio('Sigma', "RADIOCLIP", default = True, key = 'single_vel_clip', tooltip='Velocity dispersion'), sg.InputText(30, size = (4,1), key = 'clip_to_vel')],
            [sg.Radio('R and sigma vel file', "RADIOCLIP", key = 'file_for_clip',tooltip='ASCII file with R and sigma to perform sigma clipping for all the loaded spectra'), sg.InputText('txt_sample_files/sigma_clip_data.dat', size=(14, 1), key = 'sigma_clip_file'), sg.FileBrowse(tooltip='Load an ASCII file containing: Name of the spectrum, Resolution (R), sigma (km/s), in the same order of the original spectra list')],
            [sg.Checkbox('Dopcor', font = ('Helvetica', 14, 'bold'), key = 'dopcor',tooltip='Doppler correction of spectrum, from a velocity list file or from a fixed radial velocity value'), sg.Radio ('I have a file', "RADIODOP", key = 'file_for_dopcor'), sg.InputText('txt_sample_files/dopcor_file.dat', size=(14, 1), key = 'dopcor_file'), sg.FileBrowse(tooltip='Load an ASCII file containing: Name of the spectrum, radial velocity to correct (km/s), in the same order of the original spectra list')],
            [sg.Radio('Single shot', "RADIODOP", default = True, key ='dopcor_single_value'), sg.Text('Rec vel (km/s)'), sg.InputText(0, size = (8,1), key = 'dopcor_value')],
            [sg.Checkbox('Helio corr.', font = ('Helvetica', 14, 'bold'), key = 'helio_corr',tooltip='Heliocentric correction, from a formatted file or by inserting the location, time and object coordinates (RA and Dec) manually'), sg.Radio ('I have a file', "RADIOHEL", key = 'file_for_helio'), sg.InputText('txt_sample_files/file_helio.dat', size = (12,1), key = 'helio_file'), sg.FileBrowse(tooltip='Load an ASCII file containing: Location, Date (YYYY-MM-DD), RA, Dec.')],
            [sg.Radio('Single', "RADIOHEL", default = True, key = 'helio_single_value'), sg.InputText('Paranal', size = (6,1), key = 'helio_location'), sg.InputText('2016-6-4', size = (8,1), key = 'helio_date'), sg.InputText(4.88375, size = (6,1), key = 'helio_ra'), sg.InputText(35.0436389, size = (6,1), key = 'helio_dec'), sg.Button('loc.list',button_color=('black','light blue'),tooltip='Click to see the pre-loaded location list for heliocentric correction')],
            ], font=('Helvetica', 16, 'bold'), title_color = 'lightgreen'),

            #2) spectra processing
            sg.Frame('Spectra processing', [
            [sg.Checkbox('Rebin', font = ('Helvetica', 14, 'bold'), key = 'rebin',tooltip='Rebinning the spectrum, to a linear wavelength step (nm) or to a linear sigma step (km/s)'), sg.Radio('pix lin.', "RADIO1", default=True, key = 'rebin_pix_lin'), sg.InputText(0.02, size = (4,1), key = 'rebin_step_pix'), sg.Radio('sigma lin.', "RADIO1", key = 'rebin_sigma_lin'), sg.InputText(0, size = (3,1), key = 'rebin_step_sigma')],
            [sg.Checkbox('DegradeR', font = ('Helvetica', 14, 'bold'), key = 'degrade_resolution',tooltip='Degrade resolution, for initial R to a user defined R'), sg.Text('From R:'), sg.InputText(5000, size = (6,1), key = 'degrade_from_r'), sg.Text('to R:'), sg.InputText(500, size = (6,1), key = 'degrade_to_r')],
            [sg.Checkbox('DegradeL', font = ('Helvetica', 14, 'bold'), key='degrade_resolution_lambda',tooltip='Degrade resolution, from an initial wavelength FWHM to a user defined wavelength FWHM, in Angstrom'), sg.Text('from (FWHM, A):'), sg.InputText(2.51, size = (4,1), key = 'degrade_from_l'), sg.Text('to:'), sg.InputText(8.5, size = (4,1), key = 'degrade_to_l')     ],
            ...
            [sg.Checkbox('Blackbody fitting', font = ('Helvetica', 16, 'bold'), key = 'bb_fitting',tooltip='Blackdoby Planck function fitting. Works fine for stellar spectra and wide wavelength range'),sg.Push(), sg.Button('blackbody parameters',button_color= ('black','light blue'), size = (22,1))],

            #1) Cross-correlation
            [sg.Checkbox('Cross-correlation', font = ('Helvetica', 16, 'bold'), key = 'xcorr',tooltip='Cross-correlating a user defined portion of the spectrum with a template to retrieve the radial velocity. Use ppxf for more accurate results'),sg.Push(), sg.Button('Xcorr parameters',button_color= ('black','light blue'), size = (22,1))],

            #2) Velocity disperion measurement
            [sg.Checkbox('Velocity dispersion', font = ('Helvetica', 16, 'bold'), key = 'sigma_measurement',tooltip='Fitting a band with a template. Very rough, just for an estimation of the sigma. Use ppxf for science results'),sg.Push(), sg.Button('Sigma parameters',button_color= ('black','light blue'), size = (22,1))],


            #3) Equivalent width measurement
            [sg.Checkbox('Equivalent width', font = ('Helvetica', 16, 'bold'), key = 'ew_measurement',tooltip='Equivalent width measurement for a list of indices, a single, user defined, index and Lick/IDS indices'),sg.Push(), sg.Button('Equivalent width parameters',button_color= ('black','light blue'), size = (22,1))],
            #[sg.HorizontalSeparator()],


            #4) Line fitting
            [sg.Checkbox('Line(s) fitting', font = ('Helvetica', 16, 'bold'), key = 'line_fitting',tooltip='User line or automatic CaT band fitting with gaussian functions'),sg.Push(), sg.Button('Line fitting parameters',button_color= ('black','light blue'), size = (22,1))],
            #[sg.HorizontalSeparator()],

            #5) Kinematics with ppxf
            [sg.Checkbox('Kinematics with ppxf', font = ('Helvetica', 16, 'bold'), key = 'ppxf_kin',tooltip='Perform the fitting of a spectral region and gives the kinematics. Spectra need to be z corrected'),sg.Push(), sg.Button('ppxf kin parameters',button_color= ('black','light blue'), size = (22,1))  ],
            #[sg.HorizontalSeparator()],

            #6) Stellar populations with ppxf
            [sg.Checkbox('Stellar populations with ppxf', font = ('Helvetica', 16, 'bold'), key = 'ppxf_pop',tooltip='Perform the fitting of a spectral region and gives the properties of the stellar populations. Spectra need to be z corrected'),sg.Push(), sg.Button('ppxf pop parameters',button_color= ('black','light blue'), size = (22,1))  ],
            ...
            [sg.Checkbox('Sigma coeff determination', font = ('Helvetica', 16, 'bold'), text_color = 'light grey', key = 'sigma_coeff',tooltip='Determination of the velocity dispersion coefficients to correct the EW of the indices of the spectrum'),sg.Push(), sg.Button('sigma coeff parameters',button_color= ('black','light blue'), size = (22,1))],

            #8) Apply the velocity dispersion correction coefficients to the loaded spectra
            [sg.Checkbox('Correct EWs for sigma', font = ('Helvetica', 16, 'bold'), text_color = 'light grey', key = 'sigma_corr',tooltip='Correct the raw measured EW of the spectra to a zero velocity dispersion frame, using the coefficients calculated in the Sigma coeff determination task'),sg.Push(),sg.Button('sigma corr parameters',button_color= ('black','light blue'), size = (22,1)) ]
            ], font=('Helvetica', 16, 'bold'), title_color='yellow'),
    ...
    #return sg.Window('SPAN - SPectral ANalysis - 4.5 --- Daniele Gasparri ---', layout,finalize=True, scaling = 0.1)


window1 = sg.Window('SPAN - SPectral ANalysis - 4.7 --- Daniele Gasparri ---', layout,finalize=True)       # start off with 1 window open
keys = []
events = []
values = {}
...
print ('***********************************************')
print ('********* Welcome to SPAN version 4.7 *********')
...
#****************** Initializing and checking the variables of the spectra frame ******************
    ...
    if event == 'Version':
        sg.popup ('This is version 4.7 with improved and semplified layout')
    ...
    if event == 'Clear all tasks':
        ...
        window ['degrade_resolution_lambda']. Update (value = False)
        ...
        window ['moving_avg']. Update (value = False)
...
#********** Initializing and checking the variables of the Spectra pre-processing frame **********
    ...
    if sigma_clipping == True:
        ...
        try:
            clip_factor = float(values['clip_factor'])
            if clip_factor <=0:
            ...
            continue
                #check values
    sigma_clip_resolution = float(values['res_spec_for_sigma_clip'])
    sigma_clip_single_vel = values['single_vel_clip']
    sigma_clip_vel_value = float(values['clip_to_vel'])
    ...
    if dop_cor == True and dop_cor_single_shot == True:
        try:
            dop_cor_single_shot_vel = float(values['dopcor_value'])
        except ValueError:
    ...
    if helio_corr == True and helio_single_shot == True:
        try:
            ra_obj = float(values['helio_ra'])
            dec_obj = float(values['helio_dec'])
        except ValueError:
...
#************ Initializing and checking the variables of the Spectra processing frame ***********
    ...
    if degrade == True:
        try:
            original_resolution = float(values['degrade_from_r'])
            final_resolution = float(values['degrade_to_r'])
            if original_resolution <= 0 or final_resolution <= 0:
                sg.popup('Invalid resolution. Must be > 0!')
                continue
        except ValueError:
            sg.popup('Resolution not valid!')
            continue

    #4) degrade the resolution from lambda fixed and check on the input values
    degrade_lambda_fwhm = values['degrade_resolution_lambda']
    if degrade_lambda_fwhm == True:
        try:
            original_resolution_lambda = float(values['degrade_from_l'])
            final_resolution_lambda = float(values['degrade_to_l'])
            if original_resolution_lambda <= 0 or final_resolution_lambda <= 0:
                sg.popup('Invalid resolution. Must be > 0!')
                continue
        except ValueError:
            sg.popup('Resolution not valid!')
    ...
    if normalize_wave == True:
        try:
            ...
            sg.popup('Normalization wave not valid!')
            continue

    #6) rough continuum subtraction
    ...
    #9) moving average and check on the input values
    moving_average = values['moving_avg']
    if moving_average == True:
        try:
            window_size = int(values['avg_window'])
            if window_size <=0:
                sg.popup('Window size moving average. Must be > 0!')
                continue
        except ValueError:
            sg.popup('Window size not valid! Must be integer value with no letters')
            continue
...
#*************************************** SUB WINDOWS DEFINITION AND PARAMETERS ***************************************
    ...
    if (event == 'blackbody parameters'):
        sg.theme('LightBlue1')
        bb_layout = [
        [sg.Text('Wave interval:'), sg.InputText(wave1_bb, size = (4,1), key = 'left_wave_bb'), sg.Text('-'), sg.InputText(wave2_bb, size = (4,1), key = 'right_wave_bb'), sg.Text('Initial Temperature guess'), sg.InputText(t_guess, size = (4,1), key = 't_guess_bb')],
    ...
    if (event == 'Xcorr parameters'):
        sg.theme('LightBlue1')
        xcorr_layout = [
        [sg.Text('Select template: '), sg.InputText(template_crosscorr, size = (15,1), key = 'xcorr_template'), sg.FileBrowse(tooltip='Load a template'), sg.Radio ('nm', "RADIOCORR", default = lambda_units_template_crosscorr_nm, key = 'xcorr_template_wave_nm'), sg.Radio('a', "RADIOCORR", default = lambda_units_template_crosscorr_a, key = 'xcorr_template_wave_a'), sg.Radio('mu', "RADIOCORR", key = 'xcorr_template_wave_mu', default = lambda_units_template_crosscorr_mu), sg.Checkbox('Add broadening to template (km/s):', key = 'xcorr_smooth_template',tooltip='Add a sigma broadeing to the template (km/s)', default = smooth_template_crosscorr), sg.InputText(smooth_value_crosscorr, size = (4,1), key = 'xcorr_smooth_template_value'), sg.Text(' '), sg.Button('View template',button_color=('black','light blue'))],
        [sg.Radio('Wavelength interval to cross-correlate (nm):', "RADIOONLY", default = limit_range_corr, key = 'xcorr_wave_range',tooltip='Wavelength range to cross-correlate in nm'), sg.InputText(low_wave_corr, size = (5,1), key = 'xcorr_left_lambda'), sg.Text('-'), sg.InputText(high_wave_corr, size = (5,1), key = 'xcorr_right_lambda'), sg.Radio('Velocity range allowed (km/s):', "RADIOONLY2", default = vel_range_corr, key = 'xcorr_vel_range',tooltip='Velocity range, in km/s'), sg.InputText(low_vel_corr, size = (5,1), key = 'xcorr_low_vel'), sg.Text('-'), sg.InputText(high_vel_corr, size = (5,1), key = 'xcorr_high_vel')],
        [sg.Push(), sg.Button('Confirm',button_color= ('black','orange'), size = (18,1))]
        ]
...
# 2) VELOCITY DISPERSION
    ...
    if (event == 'Sigma parameters'):
        sg.theme('LightBlue1')
        sigma_layout = [
            [sg.InputText(template_sigma, size = (18,1), key = 'sigma_template'), sg.FileBrowse(tooltip='Load a template'), sg.Radio ('nm', "RADIOSIGMA", default = lambda_units_template_sigma_nm, key = 'sigma_template_wave_nm'), sg.Radio('a', "RADIOSIGMA", default = lambda_units_template_sigma_a, key = 'sigma_template_wave_a'), sg.Radio('mu', "RADIOSIGMA", default = lambda_units_template_sigma_mu, key = 'sigma_template_wave_mu'), sg.Text('Pre-loaded bands:'), sg.Radio('CaT', "RADIOBAND", default = band_cat, key = 'sigma_band_cat'), sg.Radio('Ha', "RADIOBAND", default = band_halpha, key = 'sigma_band_ha'), sg.Radio('Nad', "RADIOBAND", default = band_nad, key = 'sigma_band_nad'), sg.Radio('H band', "RADIOBAND", default = band_h, key = 'sigma_band_h'), sg.Radio('K band', "RADIOBAND", default = band_k, key = 'sigma_band_k'), sg.Text('     ', font = ("Helvetica", 12)), sg.Button('View template',button_color=('black','light blue'))],
            [sg.Text('           '), sg.Text('Resolution spectrum'), sg.InputText(resolution_spec, size=(5,1), key = 'sigma_spec_res'), sg.Text('Resolution template (0 if (E)MILES)'), sg.InputText(resolution_template, size = (5,1), key = 'sigma_res_template'), sg.Radio('Custom band', "RADIOBAND", default = band_custom, key = 'sigma_custom_band'), sg.Text('Wave interval (nm)'), sg.InputText(low_wave_sigma, size = (5,1), key = 'sigma_left_lambda'), sg.Text('-'), sg.InputText(high_wave_sigma, size = (5,1), key = 'sigma_right_lambda'), sg.Text('Cont. interval'), sg.InputText(low_wave_cont, size = (5,1), key = 'sigma_cont_left_lambda'), sg.Text('-'), sg.InputText(high_wave_cont, size = (5,1), key = 'sigma_cont_right_lambda')],
            [sg.Push(), sg.Button('Confirm',button_color= ('black','orange'), size = (18,1))]
            ]
...
#################################################################################################
#  ************************* MEGA EVENTO 1: PREMO IL TASTO PREVIEW SPECTRUM O PROCESS SELECTED! ****************************
# VALIDO PER I FRAME SPECTRA PRE-PROCESSING, PROCESSING AND MATH.
    ...
    #**************** spectra pre-processing ***********************************
        ...
        if sigma_clipping == True and sigma_clip_have_file == False:
            ...
            clip_wavelength, clip_flux = span.sigma_clip(wavelength, flux, clip_factor, sigma_clip_resolution, sigma_clip_vel_value)
...
#************************************** SPECTRA PROCESSING *************************
        #if I want to degrade the resolution
        if degrade == True:
            task_done = 1
            task_spec = 1
            if final_resolution > original_resolution:
                sg.popup('You want to improve the resolution? That''s impossible!')
                continue

            print ('*** Degrade resolution ***')

            degraded_wave, degraded_flux = span.degrade(wavelength, flux, original_resolution, final_resolution, True)
            #update the variables. necessary because I will use always wavelength and flux for the other functions!
            wavelength = degraded_wave
            flux = degraded_flux

            if event == 'Process selected' and save_intermediate_files == True:
                degraded_res = str(int(round(final_resolution)))
        ...
        #If I want to degrade the resolution in lambda (FWHM)
        if degrade_lambda_fwhm == True:
            task_done = 1
            task_spec = 1
            if final_resolution_lambda < original_resolution_lambda:
                sg.popup('You want to improve the resolution? That''s impossible!')
                continue

            print ('*** Degrade resolution in FWHM ***')

            degraded_wave, degraded_flux = span.degrade_lambda(wavelength, flux, original_resolution_lambda, final_resolution_lambda)
            #update the variables. necessary because I will use always wavelength and flux for the other functions!
            wavelength = degraded_wave
            flux = degraded_flux

            if event == 'Process selected' and save_intermediate_files == True:
                degraded_res_lambda = str(int(round(final_resolution_lambda)))
                file_degraded_lambda = result_spec+'degraded_R' + degraded_res_lambda + '_' + prev_spec_nopath + '.dat'
        ...
        if normalize_wave == True:
            ...
            if (norm_lambda < wave_limits[0] or norm_lambda > wave_limits[1]):
                sg.popup('Normalization wavelength exceeds the range of the spectrum!')
                continue
        ...
        if continuum_sub == True:
            ...
            subtracted_flux, continuum_flux = span.sub_cont(wavelength, flux)
            #wavelength = wave
        ...
        if (moving_average == True):
            ...
            avg_flux = span.mov_avg(flux, window_size)
            flux = avg_flux

            if event == 'Process selected' and save_intermediate_files == True:
                mov_avg_suffix = str(int(round(window_size)))
                file_mov_avg = result_spec+'mov_avg' + mov_avg_suffix + prev_spec_nopath + '.dat'
...
#************************************** SPEC MATH *******************************************
        ...
        if (subtract_normalized_avg == True and values['one_spec'] == False):
            task_done = 1
            task_spec = 1
            print ('*** Subract normalized average ***')
        ...
        if (subtract_normalized_spec == True):
            task_done = 1
            task_spec = 1
            print ('*** Subtract normalized spectrum ***')
        ...
        if (do_nothing == False):
           ...
           #average all
            ...
            if (norm_and_average == True and values['one_spec'] == False):
                task_done = 1
                task_spec = 1
                print ('*** Normalize and average all ***')
...
##################################### END TASKS ################################################
    ...
    #activating the button location list, must be outside the main event!
    if(event == 'loc.list'):
...
#************************************** SPECTRA PROCESSING *************************
            ...
            #5B) Degrade resolution to lambda FWHM
            if degrade_lambda_fwhm == True:
                task_done2 = 1
                task_spec2 = 1
                if i == 0:
                ...
                if (save_intermediate_files == True):
                    degraded_res_lambda = str(int(round(final_resolution_lambda)))
                    file_degraded_lambda = result_spec+'degraded_R' + degraded_res_lambda + '_' + spec_names_nopath[i] + '.dat'
                    np.savetxt(file_degraded_lambda, np.column_stack([wavelength, flux]), header="wavelength \t flux")
                    print ('File saved: ', file_degraded_lambda)
                    print('')
            ...
            if normalize_wave == True:
                ...
                task_spec2 = 1
                            #check the limits
                if (norm_lambda < wave_limits[0] or norm_lambda > wave_limits[1]):
                    print('Normalization wavelength exceeds the range of the spectrum! Skipping')
                    #continue
                else:
                    if i == 0:
                        print ('Running normalize task...')
            ...
            if (add_noise == True):
                ...
                if (save_intermediate_files == True):
                    noise_suffix = str(int(round(noise)))
                    file_noise = result_spec+'SNR' + noise_suffix + spec_names_nopath[i] + '.dat'
                    np.savetxt(file_noise, np.column_stack([wavelength, flux]), header="wavelength \t flux")
                    print ('File saved: ', file_noise)
                    print('')

            # 10) moving average: to be implemented
            if (moving_average == True):
                task_done2 = 1
                task_spec2 = 1
                if i == 0:
                    print ('Running moving average task')
                ...
                if (save_intermediate_files == True):
                    mov_avg_suffix = str(int(round(window_size)))
                    file_mov_avg = result_spec+'mov_avg' + mov_avg_suffix + spec_names_nopath[i] + '.dat'
                    np.savetxt(file_mov_avg, np.column_stack([wavelength, flux]), header="wavelength \t flux")
                    print ('File saved: ', file_mov_avg)
                    print('')
...
#******************************************* SPEC MATH ********************************************
            ...
            if (subtract_normalized_avg == True):
                ...
                task_spec2 = 1
                if i == 0:
                    print ('Running subtract normalized average task...')
            ...
            if (subtract_normalized_spec == True):
                ...
                if cond000 == True:
                    if i == 0:
                        print ('Running subtract normalized spectrum task...')
...
#************************************** SAVE AND LOAD PARAMETER VALUES *********************************************
    ...
    if event == 'Load parameters...':
        ...
        try:
            ...
            window['clip_factor'].update(value=values.get('clip_factor', ''))
            window['res_spec_for_sigma_clip'].update(value=values.get('res_spec_for_sigma_clip', ''))
            window['single_vel_clip'].update(value=values.get('single_vel_clip', False))
            window['clip_to_vel'].update(value=values.get('clip_to_vel', ''))
            window['file_for_clip'].update(value=values.get('file_for_clip', False))
            window['sigma_clip_file'].update(value=values.get('sigma_clip_file', ''))
            window['dopcor'].update(value=values.get('dopcor', False))
            window['file_for_dopcor'].update(value=values.get('file_for_dopcor', False))
            window['dopcor_file'].update(value=values.get('dopcor_file', ''))
            window['dopcor_single_value'].update(value=values.get('dopcor_single_value', False))
            window['dopcor_value'].update(value=values.get('dopcor_value', ''))
            window['helio_corr'].update(value=values.get('helio_corr', False))
            window['file_for_helio'].update(value=values.get('file_for_helio', False))
            window['helio_file'].update(value=values.get('helio_file', ''))
            window['helio_single_value'].update(value=values.get('helio_single_value', False))
            window['helio_location'].update(value=values.get('helio_location', ''))
            window['helio_date'].update(value=values.get('helio_date', ''))
            window['helio_ra'].update(value=values.get('helio_ra', ''))
            window['helio_dec'].update(value=values.get('helio_dec', ''))
            ...
            window['degrade_from_r'].update(value=values['degrade_from_r'],)
            window['degrade_to_r'].update(value=values['degrade_to_r'],)
            window['degrade_resolution_lambda'].update(value=values['degrade_resolution_lambda'],)
            window['degrade_from_l'].update(value=values['degrade_from_l'],)
            window['degrade_to_l'].update(value=values['degrade_to_l'],)
            ...
            window['moving_avg'].update(value=values['moving_avg'],)
            window['avg_window'].update(value=values['avg_window'],)
    ...
    if event == 'Restore default parameters':
        ...
        window['wave_units_a'].update(value=values.get('wave_units_a', False))
        window['wave_units_mu'].update(value=values.get('wave_units_mu', False))
        #window['listfile'].update(value=values.get('listfile', False))
        window['show_hdr'].update(value=values.get('show_hdr', False))
        window['show_step'].update(value=values.get('show_step', False))
        window['show_res'].update(value=values.get('show_res', False))
        window['lambda_res_left'].update(value=values.get('lambda_res_left', ''))
        ...
        wave1_kin=values['left_wave_ppxf_kin']
        wave2_kin=values['right_wave_ppxf_kin']
        #values['markers_ppxf_kin']=values['#'markers_ppxf_kin']
        stellar_library_kin = values['stellar_library_kin']
        constant_resolution_lambda=values['constant_resolution_lambda']
        resolution_kin=values['ppxf_resolution']
        constant_resolution_r=values['constant_resolution_r']
        ...
        additive_degree=values['additive_degree']
        multiplicative_degree=values['multiplicative_degree']
        #values['markers_ppxf']=values['#'markers_ppxf']
        stellar_library = values['stellar_library']
        with_errors=values['ppxf_err_pop']
        have_index_file_corr=values['ew_corr_idx_file']
        index_file_corr=values['idx_corr_file']

...
layout = [
            ...
            [sg.Checkbox('Convert Flux', font = ('Helvetica', 10, 'bold'), key = 'convert_flux',tooltip='Convert the flux from Jansky to F_lambda and viceversa'), sg.Radio('Jy-->F_nu', "FLUX", default = True, key = 'convert_to_fnu'), sg.Radio('Jy-->F_l', "FLUX", key = 'convert_to_fl'),sg.Button('See plot',button_color=('black','light gray')), sg.Text(' ', font = ('Helvetica', 5)) ],
            [sg.Checkbox('SNR:', font = ('Helvetica', 10, 'bold'), key = 'show_snr',tooltip='Show the S/N of the spectrum/spectra centered on an user defined wavelength(W)'), sg.Text(' W.'), sg.InputText('609', size = (4,1), key = 'wave_snr'), sg.Text('+/-'), sg.InputText(3, size = (3,1), key = 'delta_wave_snr'), sg.Button('Save one',button_color=('black','light gray')), sg.Button('Save all',button_color=('black','light gray'))]  ], font=("Helvetica", 12, 'bold'),tooltip='This is a standalone frame. Use the buttons on the right to perform the tasks'),
            ...
            [sg.Checkbox('Clean', font = ('Helvetica', 10, 'bold'), key = 'sigma_clip',tooltip='Perform sigma clipping to erase spikes'), sg.Text('Clip ',tooltip='Clipping factor'), sg.InputText(2.5, size = (5,1), key = 'clip_factor'), sg.Text('Res. (R)',tooltip='Spectrum resolution'), sg.InputText('1600', size = (5,1), key = 'res_spec_for_sigma_clip'), sg.Radio('Sigma', "RADIOCLIP", default = True, key = 'single_vel_clip', tooltip='Velocity dispersion'), sg.InputText(30, size = (5,1), key = 'clip_to_vel')],
            [sg.Radio('R and sigma vel file', "RADIOCLIP", key = 'file_for_clip',tooltip='ASCII file with R and sigma to perform sigma clipping for all the loaded spectra'), sg.InputText('txt_sample_files/sigma_clip_data.dat', size=(18, 1), key = 'sigma_clip_file'), sg.FileBrowse(tooltip='Load an ASCII file containing: Name of the spectrum, Resolution (R), sigma (km/s), in the same order of the original spectra list')],
            [sg.Checkbox('Dopcor', font = ('Helvetica', 10, 'bold'), key = 'dopcor',tooltip='Doppler correction of spectrum, from a velocity list file or from a fixed radial velocity value'), sg.Radio ('I have a file', "RADIODOP", key = 'file_for_dopcor'), sg.InputText('txt_sample_files/dopcor_file.dat', size=(16, 1), key = 'dopcor_file'), sg.FileBrowse(tooltip='Load an ASCII file containing: Name of the spectrum, radial velocity to correct (km/s), in the same order of the original spectra list')],
            ...
            [sg.Checkbox('Helio corr.', font = ('Helvetica', 10, 'bold'), key = 'helio_corr',tooltip='Heliocentric correction, from a formatted file or by inserting the location, time and object coordinates (RA and Dec) manually'), sg.Radio ('I have a file', "RADIOHEL", key = 'file_for_helio'), sg.InputText('txt_sample_files/file_helio.dat', size = (15,1), key = 'helio_file'), sg.FileBrowse(tooltip='Load an ASCII file containing: Location, Date (YYYY-MM-DD), RA, Dec.')],
            [sg.Radio('Single', "RADIOHEL", default = True, key = 'helio_single_value'), sg.InputText('Paranal', size = (7,1), key = 'helio_location'), sg.InputText('2016-6-4', size = (8,1), key = 'helio_date'), sg.InputText(4.88375, size = (7,1), key = 'helio_ra'), sg.InputText(35.0436389, size = (7,1), key = 'helio_dec'), sg.Button('loc.list',button_color=('black','light blue'),tooltip='Click to see the pre-loaded location list for heliocentric correction')],
            ], font=("Helvetica", 12, 'bold'), title_color = 'lightgreen'),

            #2) spectra processing
            sg.Frame('Spectra processing', [
            [sg.Checkbox('Rebin', font = ('Helvetica', 10, 'bold'), key = 'rebin',tooltip='Rebinning the spectrum, to a linear wavelength step (nm) or to a linear sigma step (km/s)'), sg.Radio('pix lin.', "RADIO1", default=True, key = 'rebin_pix_lin'), sg.InputText(0.02, size = (6,1), key = 'rebin_step_pix'), sg.Radio('sigma lin.', "RADIO1", key = 'rebin_sigma_lin'), sg.InputText(0, size = (5,1), key = 'rebin_step_sigma')],
            [sg.Checkbox('DegradeR', font = ('Helvetica', 10, 'bold'), key = 'degrade_resolution',tooltip='Degrade resolution, for initial R to a user defined R'), sg.Text('From R:'), sg.InputText(1600, size = (7,1), key = 'degrade_from_r'), sg.Text('to R:'), sg.InputText(600, size = (7,1), key = 'degrade_to_r')],
            [sg.Checkbox('DegradeL', font = ('Helvetica', 10, 'bold'), key='degrade_resolution_lambda',tooltip='Degrade resolution, from an initial wavelength FWHM to a user defined wavelength FWHM, in Angstrom'), sg.Text('from (FWHM, A):'), sg.InputText(2.51, size = (5,1), key = 'degrade_from_l'), sg.Text('to:'), sg.InputText(8.5, size = (5,1), key = 'degrade_to_l')     ],
            [sg.Checkbox('Normalize to:', font = ('Helvetica', 10, 'bold'), key = 'norm_spec',tooltip='Normalize the flux to a user defined wavelength'), sg.InputText(500, size = (6,1), key = 'norm_wave'), sg.Text('nm') , sg.Checkbox('Cont. removal', font = ('Helvetica', 10, 'bold'), key = 'cont_sub',tooltip='Perform a rough continuum subtraction of the spectrum')],
            [sg.Checkbox('Sigma broadening', font = ('Helvetica', 10, 'bold'), key = 'broadening_spec',tooltip='Broad the spectra by adding a user defined sigma (km/s)'), sg.Text('Add sigma (km/s): ', font = ('Helvetica', 10)), sg.InputText(0, size = (6,1), key = 'sigma_to_add')],
            [sg.Checkbox('Add noise', font = ('Helvetica', 10, 'bold'), key = 'add_noise',tooltip='Adding poissonian noise to the spectrum'), sg.Text('SNR:'), sg.InputText(10, size = (4,1), key = 'noise_to_add'), sg.Checkbox('Mov avg', font = ('Helvetica', 10, 'bold'), key = 'moving_avg',tooltip='Moving average with a user defined box window, in pixel'), sg.Text('pix:'), sg.InputText(11, size = (3,1), key = 'avg_window')],
            ...
            [sg.Checkbox('Subtract normalized average', font = ('Helvetica', 10, 'bold'), key = 'subtract_norm_avg',tooltip='Normalize and subtract to the selected spectrum the normalized average of all the spectra')],
            [sg.Checkbox('Subtract norm. spec.', font = ('Helvetica', 10, 'bold'), key = 'subtract_norm_spec',tooltip='Normalize and subtract to the selected spectrum a user selected spectrum'), sg.InputText('Spectra to subtract', size = (17,1), key = 'spec_to_sobtract'), sg.FileBrowse(tooltip='Load a spectrum (ASCII or fits) to be normalized and subtracted'), sg.Text(' ')],
            ...
            #1) Cross-correlation
            [sg.Checkbox('Cross-correlation', font = ('Helvetica', 11, 'bold'), key = 'xcorr',tooltip='Cross-correlating a user defined portion of the spectrum with a template to retrieve the radial velocity. Use ppxf for more accurate results'),sg.Push(), sg.Button('Xcorr parameters',button_color= ('black','light blue'), size = (22,1))],

            #2) Velocity disperion measurement
            [sg.Checkbox('Velocity dispersion', font = ('Helvetica', 11, 'bold'), key = 'sigma_measurement',tooltip='Fitting a band with a template. Very rough, just for an estimation of the sigma. Use ppxf for science results'),sg.Push(), sg.Button('Sigma parameters',button_color= ('black','light blue'), size = (22,1))],


            #3) Equivalent width measurement
            [sg.Checkbox('Equivalent width', font = ('Helvetica', 11, 'bold'), key = 'ew_measurement',tooltip='Equivalent width measurement for a list of indices, a single, user defined, index and Lick/IDS indices'),sg.Push(), sg.Button('Equivalent width parameters',button_color= ('black','light blue'), size = (22,1))],
            #[sg.HorizontalSeparator()],
            ...
            [sg.Checkbox('Kinematics with ppxf', font = ('Helvetica', 11, 'bold'), key = 'ppxf_kin',tooltip='Perform the fitting of a spectral region and gives the kinematics. Spectra need to be z corrected'),sg.Push(), sg.Button('ppxf kin parameters',button_color= ('black','light blue'), size = (22,1))  ],
            #[sg.HorizontalSeparator()],

            #6) Stellar populations with ppxf
            [sg.Checkbox('Stellar populations with ppxf', font = ('Helvetica', 11, 'bold'), key = 'ppxf_pop',tooltip='Perform the fitting of a spectral region and gives the properties of the stellar populations. Spectra need to be z corrected'),sg.Push(), sg.Button('ppxf pop parameters',button_color= ('black','light blue'), size = (22,1))  ],
            ...
            sg.Frame('Output', [
            [sg.Output(size=(82, 17), key='-OUTPUT-' , font=('Helvetica', 11))],
            ] ,font=("Helvetica", 12, 'bold')),
...
window1 = sg.Window('SPAN - SPectral ANalysis - 4.7 --- Daniele Gasparri ---', layout,finalize=True, scaling = scale_win)       # start off with 1 window open
keys = []
events = []
values = {}
